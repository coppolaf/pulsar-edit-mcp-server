...
import { handleSendMessage, fetchModels, clearContextHistory } from './chat-functions';
import { applyPreferredModelSelection } from './chat/model-selection';
...
export default class ChatPanel {
  ...
  constructor(serializedState) {
    ...
    fetchModels()
      .then((models) => {
        ...
        applyPreferredModelSelection(modelSelect, models);
      })
  ...
  handleSend() {
    ...
    handleSendMessage(this, chatDisplay, marked, DOMPurify, message, model, this.mcpClient)
      .catch((err) => this.showError(err.message || 'Unexpected error'))
      .finally(() => {
        this.pendingSendCount = Math.max(0, this.pendingSendCount - 1);
        this.updateSendButtonState();
      });
  ...
  updateSendButtonState() {
    ...
    this.sendButton.disabled = this.pendingSendCount > 0;
    this.sendButton.textContent = this.pendingSendCount > 0 ? 'Queued…' : 'Send';
  }
