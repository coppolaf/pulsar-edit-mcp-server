...
export function createMcpHttpRuntime({ version, port, serverName = 'pulsar-edit-mcp-server-server' }) {
  ...
  function disposeSessionRegistration(sessionId) {
    const registration = sessionRegistrations[sessionId];
    if (registration && typeof registration.dispose === 'function') {
      registration.dispose();
    }
    delete sessionRegistrations[sessionId];
  }
    ...
    if (sessionId && transports[sessionId]) {
      ...
      transport.onclose = () => {
        if (transport.sessionId) {
          ...
          disposeSessionRegistration(transport.sessionId);
        }
      };
      ...
      const registration = mcpRegistration(mcpServer);
      await mcpServer.connect(transport);

      if (transport.sessionId) {
        sessionRegistrations[transport.sessionId] = registration;
      }
    } else {
      res.status(400).json({
        jsonrpc: '2.0',
      ...
      return new Promise((resolve) => {
        ...
        currentInstance.close(() => {
          Object.keys(transports).forEach((sessionId) => {
            ...
            disposeSessionRegistration(sessionId);
          });
          resolve(true);
        });
      });
