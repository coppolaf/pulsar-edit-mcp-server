...
import { z } from 'zod';
import { createDocumentWriteService, WRITE_EXECUTION_MODES, WRITE_POLICY_MODES } from './document-write-service.js';

const executionModeSchema = z.enum([WRITE_EXECUTION_MODES.PROPOSE, WRITE_EXECUTION_MODES.APPLY]).optional();

function buildExecutionModeDescription(baseDescription, { proposalSupported = true } = {}) {
  const policyDescription = proposalSupported
    ? 'Write policy: this tool supports both `apply` (mutate immediately) and `propose` (store a pending proposal for later inspection and `apply-proposal`).'
    : 'Write policy: this tool is immediate-only and does not support `executionMode=propose` because it depends on editor history state.';

  return [baseDescription, policyDescription].join(' ');
}

function formatWriteResult(result) {
  if (result.mode === WRITE_EXECUTION_MODES.PROPOSE && result.proposal) {
    const previewStats = result.proposal.preview?.diff
      ? `diffSegments: ${result.proposal.preview.diff.changedSegmentCount}, addedLines: ${result.proposal.preview.diff.addedLineCount}, removedLines: ${result.proposal.preview.diff.removedLineCount}`
      : 'diffSegments: n/a';
    ...
    return [
      ...
      `policy: ${result.proposal.policy}`,
      `file: ${result.proposal.document?.fullPath || '[unknown]'}`,
...
function registerProposalInspectionTools(server, documentWriteService) {
  registerGetPendingProposalsTool(server, documentWriteService);
  registerGetProposalTool(server, documentWriteService);
  registerGetWritePolicyTool(server, documentWriteService);
}
...
export function registerWriteTools(server, { diffHighlighter, documentWriteService = createDocumentWriteService({ diffHighlighter }) }) {
  ...
  registerProposalInspectionTools(server, documentWriteService);
...
function registerUndoTool(server, documentWriteService) {
  ...
  server.registerTool(
    ...
    {
      title: 'Undo',
      description: buildExecutionModeDescription('Undo the last change in the active editor.', { proposalSupported: false }),
...
function registerRedoTool(server, documentWriteService) {
  ...
  server.registerTool(
    ...
    {
      title: 'Redo',
      description: buildExecutionModeDescription('Redo the last undone change in the active editor.', { proposalSupported: false }),
      inputSchema: { executionMode: executionModeSchema }
    },
    async ({ executionMode } = {}) => {
      console.log(`CMD: ${curTool}, ARGS: ${JSON.stringify({ executionMode })}`);
...
function registerGetPendingProposalsTool(server, documentWriteService) {
  const curTool = 'get-pending-proposals';
  console.log('Registering Tool: ' + curTool);
  ...
  server.registerTool(
    curTool,
    {
      ...
      description: 'List all currently pending write proposals with compact metadata suitable for approval flows or proposal browsers.',
      inputSchema: {}
    },
    async () => {
      const proposals = documentWriteService.listPendingProposals().map((proposal) => ({
        id: proposal.id,
        kind: proposal.kind,
        ...
        preview: proposal.preview?.diff
          ? {
              changedSegmentCount: proposal.preview.diff.changedSegmentCount,
              addedLineCount: proposal.preview.diff.addedLineCount,
              removedLineCount: proposal.preview.diff.removedLineCount,
              truncated: proposal.preview.diff.truncated
            }
          : null
      }));
...
function registerGetProposalTool(server, documentWriteService) {
  const curTool = 'get-proposal';
  console.log('Registering Tool: ' + curTool);
  ...
  server.registerTool(
    ...
    {
      title: 'Get Proposal',
      description: 'Return the full stored payload for a pending proposal, including enriched preview metadata for future diff/UI rendering.',
      inputSchema: { proposalId: z.string() }
    },
    async ({ proposalId }) => {
      const proposal = documentWriteService.getProposal({ proposalId });
      return {
...
function registerGetWritePolicyTool(server, documentWriteService) {
  const curTool = 'get-write-policy';
  console.log('Registering Tool: ' + curTool);

  server.registerTool(
    curTool,
    {
      title: 'Get Write Policy',
      description: 'Return the explicit write-policy surface for MCP write tools, including which operations support proposal/apply versus immediate-only execution.',
      inputSchema: {}
    },
    async () => {
      return {
        content: [{ type: 'text', text: JSON.stringify(documentWriteService.getWritePolicySummary(), null, 2) }]
      };
    }
  );
}
